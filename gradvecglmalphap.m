function varargout=gradvecglmalphap(TH,L,rnew,rold,sord)
% [G,V]=gradvecglmalphap(TH,Lmax,rnew,rold)
% 
% Construction of gradient vector Slepian functions
%
% Parallel version of gradvecglmalpha
  ...
  if ~isstr(TH) && length(TH)==1 % POLAR CAPS
    ...
    if lp
      fname=fullfile(getenv('IFILES'),'GRADVECGLMALPHA',...
		     sprintf('gradvecglmalpha-%i-%i.mat',TH,L));
    elseif bp
      fname=fullfile(getenv('IFILES'),'GRADVECGLMALPHA',...
		     sprintf('gradvecglmalphabl-%i-%i-%i.mat',TH,L(1),L(2)));
    else
...
%     difer(ems(R1)-EM,[],[],mesg)
    ...
    save(fname,'G','V','EL','EM','N')  
        ...
        parfor mm=1:maxL+1
            m=mm-1;
            [E,Vpp,Np,th,Cp]=gradvecsdwcap(TH,L,m,0,-1);
            Vp{mm}=Vpp;
            C{mm}=Cp;
        end
